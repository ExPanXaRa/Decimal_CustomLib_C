...
int s21_handle_sub(s21_decimal value_a, s21_decimal value_b,
                   s21_decimal *result) {
  int err = OK_RETURN;
  ...
  s21_long_decimal res = s21_long_decimal_binary_sub(value_al, value_bl);
...
int s21_sub(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (!s21_get_sign(value_1) && !s21_get_sign(value_2)) {
    if (s21_is_greater_or_equal(value_1, value_2)) {
      err = s21_handle_sub(value_1, value_2, &tmp);
    } else {
      err = s21_handle_sub(value_2, value_1, &tmp);
      s21_negate(tmp, &tmp);
    }
    ...
    err = s21_add(value_1, tmp_val2, &tmp);
  } else if (s21_get_sign(value_1) && !s21_get_sign(value_2)) {
    s21_decimal tmp_val1 = value_1;
    s21_set_sign(&tmp_val1, 0);

    err = s21_add(tmp_val1, value_2, &tmp);
    s21_negate(tmp, &tmp);
  } else if (s21_get_sign(value_1) && s21_get_sign(value_2)) {
    s21_decimal tmp_val1 = value_1;
    s21_decimal tmp_val2 = value_2;
    ...
    if (s21_is_greater_or_equal(value_1, value_2)) {
      err = s21_handle_sub(tmp_val2, tmp_val1, &tmp);
    } else {
      err = s21_handle_sub(tmp_val1, tmp_val2, &tmp);
      s21_negate(tmp, &tmp);
    }
  }
